...
def active_membership_status_clause():
    """Compatibility predicate for rows written by older Windows builds."""
    return or_(
...
def effective_sessions_total(member_package: MemberPackage, package: Package) -> int | None:
...
def effective_sessions_remaining(
    member_package: MemberPackage, package: Package
) -> int | None:
    if member_package.sessions_remaining is not None:
        return int(member_package.sessions_remaining)
    if member_package.sessions_total is not None:
...
def active_member_package_row(db: Session, member_id: int, *, lock: bool = False):
    """Return the primary active membership shown on the member dashboard.

    GYM/COMBO has priority over a separate PT add-on. This prevents a PT row
    from hiding the member's 26-entry GYM package.
    """
    ...
    stmt = (
        ...
        .order_by(
            case(
                (Package.package_type.in_(["GYM", "COMBO"]), 0),
                else_=1,
            ),
...
def active_pt_package_row(db: Session, member_id: int):
    trainer_user = aliased(User)
    today = today_local()
    stmt = (
        select(MemberPackage, Package, Trainer, trainer_user)
        ...
        .order_by(
            (MemberPackage.pt_id.is_not(None)).desc(),
            MemberPackage.start_date.desc(),
            MemberPackage.id.desc(),
        )
        .limit(1)
...
def build_member_dashboard(db: Session, user_id: int) -> dict:
    ...
    pt_package_row = active_pt_package_row(db, member.id)
    ...
    if package_row is None:
        membership = {
            ...
            "session_label": "Số buổi còn lại",
            "progress": 0.0,
            "trainer_name": None,
            "pt_package_name": None,
            "pt_sessions_total": None,
            "pt_sessions_remaining": None,
            "pt_end_date": None,
            "status": "Chưa có gói",
        }
    else:
        member_package, package, _trainer, primary_trainer_user = package_row
        ...
        pt_remaining = None
        pt_end_date = None
        trainer_name = primary_trainer_user.full_name if primary_trainer_user else None
        if pt_package_row is not None:
            pt_member_package, pt_package, _pt_trainer, pt_trainer_user = pt_package_row
            pt_name = pt_package.name
            pt_total = effective_sessions_total(pt_member_package, pt_package)
            pt_remaining = effective_sessions_remaining(pt_member_package, pt_package)
            pt_end_date = pt_member_package.end_date.isoformat()
            if pt_trainer_user:
                trainer_name = pt_trainer_user.full_name
        ...
        membership = {
            ...
            "sessions_total": effective_sessions_total(member_package, package),
            "sessions_remaining": effective_sessions_remaining(member_package, package),
            "session_label": (
                "Lượt tập còn lại"
                if str(package.package_type or "GYM").upper() == "GYM"
                else "Buổi PT còn lại"
            ),
            "progress": round(progress, 4),
            "trainer_name": trainer_name,
            "pt_package_name": pt_name,
            "pt_sessions_total": pt_total,
            "pt_sessions_remaining": pt_remaining,
