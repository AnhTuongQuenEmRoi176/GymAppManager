    ...
    if record.entity_type == "member":
        ...
        if row is None:
            ...
            if sessions_remaining is not None and sessions_remaining <= 0:
                result["can_confirm"] = False
                result["reason"] = (
                    "Gói đã hết lượt sử dụng."
                    if package.package_type == "GYM"
                    else "Gói PT đã hết số buổi."
                )
            result["membership"] = {
...
def confirm_single_checkin(
    ...
    *,
    ...
    # Hội viên check-in một mình: chỉ trừ lượt của gói GYM giới hạn.
    # Gói PT/COMBO chỉ bị trừ khi quét đôi cùng PT.
    remaining_after_checkin = None
    if record.entity_type == "member":
        primary = active_member_package_row(db, record.entity_id, lock=True)
        if primary is not None:
            member_package, package, _trainer, _trainer_user = primary
            package_type = str(package.package_type or "GYM").upper()
            if package_type == "GYM":
                total = effective_sessions_total(member_package, package)
                remaining = effective_sessions_remaining(member_package, package)
                if remaining is not None:
                    if remaining <= 0 and not manual_override:
                        raise HTTPException(
                            status_code=409,
                            detail="Gói GYM đã hết lượt sử dụng.",
                        )
                    if member_package.sessions_total is None:
                        member_package.sessions_total = total
                    if member_package.sessions_remaining is None:
                        member_package.sessions_remaining = remaining
                    if member_package.sessions_remaining > 0:
                        member_package.sessions_remaining -= 1
                    remaining_after_checkin = member_package.sessions_remaining
    ...
    return {
        ...
        "sessions_remaining": remaining_after_checkin,
    }
...
def confirm_pair_checkin(
    ...
    *,
    ...
    effective_remaining = effective_sessions_remaining(member_package, package)
    if member_package.sessions_remaining is None and effective_remaining is not None:
        # Repair a legacy row in the same locked transaction before decrementing.
        member_package.sessions_remaining = effective_remaining
        if member_package.sessions_total is None:
            member_package.sessions_total = effective_sessions_total(member_package, package)
    if effective_remaining is None and not manual_override:
        raise HTTPException(
            status_code=409,
            detail="Gói hiện tại không có số buổi PT để xác nhận buổi tập đôi.",
        )
    if (effective_remaining or 0) <= 0 and not manual_override:
