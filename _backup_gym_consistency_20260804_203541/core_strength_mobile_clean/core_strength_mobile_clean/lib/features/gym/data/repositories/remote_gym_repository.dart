...
class RemoteGymRepository implements GymRepository {
  ...
  MembershipSummary _parseMembership(Map<String, dynamic> map) {
    ...
    return MembershipSummary(
      packageName: map['package_name']?.toString() ?? 'Chưa có gói',
      packageType: map['package_type']?.toString() ?? 'GYM',
      startDate: _asDate(map['start_date']),
      endDate: _asDate(map['end_date']),
      sessionsTotal: nullableInt(map['sessions_total']),
      sessionsRemaining: nullableInt(map['sessions_remaining']),
      sessionLabel: map['session_label']?.toString() ?? 'Số buổi còn lại',
      progress: _asDouble(map['progress']).clamp(0.0, 1.0).toDouble(),
      trainerName: map['trainer_name']?.toString(),
      ptPackageName: map['pt_package_name']?.toString(),
      ptSessionsTotal: nullableInt(map['pt_sessions_total']),
      ptSessionsRemaining: nullableInt(map['pt_sessions_remaining']),
