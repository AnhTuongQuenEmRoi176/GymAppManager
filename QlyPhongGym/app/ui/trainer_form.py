...
class TrainerForm(QDialog):
    def __init__(self, trainer_id=None):
        ...
        layout.addWidget(page_title("Thông tin PT", "Hồ sơ, bộ môn và lương cứng"))
    ...
    def save(self):
        ...
        base_salary = self.base_salary.text().strip() or "0"
        ...
        try:
            if self.trainer_id:
                ...
                user = User(username=f"pt_{int(datetime.utcnow().timestamp())}", password_hash=hash_password("pt123"), full_name=name, phone=phone)
                session.add(user)
                session.flush()
            ...
            try:
                trainer.start_date = datetime.fromisoformat(start_date).date() if start_date else None
            except Exception:
                QMessageBox.warning(self, "Lỗi", "Ngày vào phải theo định dạng YYYY-MM-DD")
                return
            trainer.base_salary = float(base_salary)
