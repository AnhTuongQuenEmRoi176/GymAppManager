...
class TrainerDetailDialog(QDialog):
    def __init__(self, trainer_id):
        ...
        try:
            ...
            layout.addWidget(page_title("Chi tiết PT", trainer.user.full_name or trainer.user.username or f"PT {trainer.id}"))

            active_packages = session.query(MemberPackage).filter(MemberPackage.pt_id == trainer.id, MemberPackage.end_date >= date.today()).count()
            total_sessions = session.query(PTSession).filter(PTSession.trainer_id == trainer.id).count()

            summary = QHBoxLayout()
            summary.addWidget(self._metric("Tổng buổi PT", total_sessions))
            summary.addWidget(self._metric("Gói PT hiện tại", active_packages))
            ...
            title = QLabel("Buổi PT đã dạy")
            title.setObjectName("sectionLabel")
            layout.addWidget(title)

            table = QTableWidget()
            table.setColumnCount(3)
            table.setHorizontalHeaderLabels(["Ngày", "Hội viên", "Xác nhận bởi"])
            configure_table(table)
            pt_sessions = session.query(PTSession).filter(PTSession.trainer_id == trainer.id).order_by(PTSession.session_date.desc()).all()
            table.setRowCount(len(pt_sessions))
            for row, pt_session in enumerate(pt_sessions):
                ...
                values = [pt_session.session_date, member_name, confirmed_name]
